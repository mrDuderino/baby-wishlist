  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "rounded-card border-border/80 bg-popover text-popover-foreground shadow-soft data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-6 border p-6 text-sm duration-200 outline-none sm:max-w-lg",
          className,
        )}
        {...props}
      >
...
function DialogTitle({ className, ...props }: DialogPrimitive.Title.Props) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn(
        "font-heading text-2xl leading-tight font-medium",
        className,
      )}
      {...props}
    />
